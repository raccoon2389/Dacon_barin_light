...
test_src = test.loc[:,'650_src':'990_src'].values
test_rho = test.loc[:,'rho'].values
test_dst = test.loc[:,'650_dst':'990_dst'].values
'''
'''
...
for t, v in skf.split(x_train):
    ...
    train_src = x_t.loc[:,'650_src':'990_src'].values
    train_dst = x_t.loc[:,'650_dst':'990_dst'].values
    # valid_rho = x_v.loc[:,'rho']
    valid_src = x_v.loc[:,'650_src':'990_src'].values
    valid_dst = x_v.loc[:,'650_dst':'990_dst'].values
    
    train_dst = train_dst.reshape(-1,35,1)
    test_dst = test_dst.reshape(-1,35,1)
    valid_dst = valid_dst.reshape(-1,35,1)
    train_src = train_src.reshape(-1,35,1)
    test_src = test_src.reshape(-1,35,1)
    valid_src = valid_src.reshape(-1,35,1)

    # rho = Input(shape = (1,))
    dst = Input(shape= (35,1))
    src = Input(shape= (35,1))

    dense1 = LSTM(30,activation='tanh')(dst)
    dense1 = Dropout(0.4)(dense1)
    dense2 = LSTM(30,activation='tanh')(src)
